  ...
  <div>
    <v-card-text class="pl-4 pr-4">
      <div class="tool a-0 ma-0">
        ...
        >
          <v-icon v-if="addressUp !== ''" small color="indigo lighten-1"
            >edit</v-icon
          >
          <v-icon v-else small color="indigo lighten-1"
            >mdi-plus-circle-outline</v-icon
          >
        </v-btn>
      </div>
      <div v-if="addressUp === ''" class="body-2">...</div>
    ...
    <div v-if="showEditView" class="pt-2 pl-2 pr-2 pb-2 indigo lighten-5">
      <v-card-text class="indigo lighten-5">
        <v-text-field v-model="addressUpdate.line" label="Address Line" />
        <v-layout row wrap>
          <v-flex>
            <v-text-field v-model="addressUpdate.city" label="Town/City" />
          </v-flex>
          <v-flex>
            <v-text-field
      ...
      <v-card-actions>
        <v-btn :disabled="!enableSave" class="ml-4" small @click="cancelEdit();"
          >CANCEL</v-btn
        >
        <v-spacer />
